...
mod connection_limit_enforcement_properties {
    ...
    impl MockConnectionLimitManager {
        ...
        fn attempt_connection(
            ...
            let total_active_connections = connections.values()
            ...
            if total_active_connections >= self.limits.global_max_connections {
                return Err(format!(
                    ...
                    self.limits.global_max_connections
                ));
            }
            
            // Check tenant connection limit
            let tenant_active_connections = connections.values()
            ...
            if tenant_active_connections >= self.limits.max_connections_per_tenant {
                return Err(format!(
                    ...
                    self.limits.max_connections_per_tenant
                ));
            }
            
            // Check project connection limit
            let project_active_connections = connections.values()
                .filter(|conn| conn.is_active && conn.tenant_id == tenant_id && conn.project_id == project_id)
                .count() as u32;
            
            if project_active_connections >= self.limits.max_connections_per_project {
                return Err(format!(
                    "Project connection limit exceeded: {}/{}",
                    project_active_connections,
                    self.limits.max_connections_per_project
                ));
            }
